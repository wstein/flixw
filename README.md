...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.31.4/flixw-setup.java
...
```
cb3119e8f12d7777cd92fbad1e9da85cca27f6eb346433d3aa07aaf4c373e8e2  flixw-setup.java
```
...
```console
echo "cb3119e8f12d7777cd92fbad1e9da85cca27f6eb346433d3aa07aaf4c373e8e2  flixw-setup.java" \
...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.31.4/SHA256SUMS
...
Invoke-WebRequest -OutFile flixw-setup.java `
  https://github.com/wstein/flixw/releases/download/v0.31.4/flixw-setup.java
...
$ ./flixw validate
ok    ./flixw matches flixw 0.31.4
ok    ./flixw.cmd matches flixw 0.31.4
...
```console
base=https://github.com/wstein/flixw/releases/download/v0.31.4
curl -fsSLO $base/flixw-0.31.4.tar.gz
curl -fsSL  $base/SHA256SUMS | grep flixw-0.31.4.tar.gz | sha256sum -c -
tar -xzf flixw-0.31.4.tar.gz        # flixw, flixw.cmd, .flixw/flixw.java
rm flixw-0.31.4.tar.gz
...
On Windows the equivalents are `Get-FileHash flixw-0.31.4.tar.gz` and `Expand-Archive` in
PowerShell, or `certutil -hashfile flixw-0.31.4.tar.gz SHA256` and `tar -xf` in cmd.exe —
...
```console
$ ./flixw pin --refresh
flixw: rewrote .flixw/lock.toml in the shape flixw 0.31.4 writes; the pin is unchanged
```
