...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.34.7/flixw-setup.java
...
```
fbdff6d21704340f783a123cc0be8d4d490ef29d978d1c3d7757bd493d84c639  flixw-setup.java
```
...
```console
echo "fbdff6d21704340f783a123cc0be8d4d490ef29d978d1c3d7757bd493d84c639  flixw-setup.java" \
...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.34.7/SHA256SUMS
...
Invoke-WebRequest -OutFile flixw-setup.java `
  https://github.com/wstein/flixw/releases/download/v0.34.7/flixw-setup.java
...
$ ./flixw validate
ok    ./flixw matches flixw 0.34.7
ok    ./flixw.cmd matches flixw 0.34.7
...
```console
base=https://github.com/wstein/flixw/releases/download/v0.34.7
curl -fsSLO $base/flixw-0.34.7.tar.gz
curl -fsSL  $base/SHA256SUMS | grep flixw-0.34.7.tar.gz | sha256sum -c -
tar -xzf flixw-0.34.7.tar.gz        # flixw, flixw.cmd, .flixw/flixw.java
rm flixw-0.34.7.tar.gz
...
On Windows the equivalents are `Get-FileHash flixw-0.34.7.tar.gz` and `Expand-Archive` in
PowerShell, or `certutil -hashfile flixw-0.34.7.tar.gz SHA256` and `tar -xf` in cmd.exe —
...
```console
$ ./flixw pin --refresh
flixw: rewrote .flixw/lock.toml in the shape flixw 0.34.7 writes; the pin is unchanged
```
