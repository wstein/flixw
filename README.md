...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.31.1/flixw-setup.java
...
```
efbe404d87af365535408bee730116db8fec3c252256ad0d920c1f49836009d1  flixw-setup.java
```
...
```console
echo "efbe404d87af365535408bee730116db8fec3c252256ad0d920c1f49836009d1  flixw-setup.java" \
...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.31.1/SHA256SUMS
...
Invoke-WebRequest -OutFile flixw-setup.java `
  https://github.com/wstein/flixw/releases/download/v0.31.1/flixw-setup.java
...
$ ./flixw validate
ok    ./flixw matches flixw 0.31.1
ok    ./flixw.cmd matches flixw 0.31.1
...
```console
base=https://github.com/wstein/flixw/releases/download/v0.31.1
curl -fsSLO $base/flixw-0.31.1.tar.gz
curl -fsSL  $base/SHA256SUMS | grep flixw-0.31.1.tar.gz | sha256sum -c -
tar -xzf flixw-0.31.1.tar.gz        # flixw, flixw.cmd, .flixw/flixw.java
rm flixw-0.31.1.tar.gz
...
On Windows the equivalents are `Get-FileHash flixw-0.31.1.tar.gz` and `Expand-Archive` in
PowerShell, or `certutil -hashfile flixw-0.31.1.tar.gz SHA256` and `tar -xf` in cmd.exe —
...
```console
$ ./flixw pin --refresh
flixw: rewrote .flixw/lock.toml in the shape flixw 0.31.1 writes; the pin is unchanged
```
