...
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.22.0/flixw.java
...
Invoke-WebRequest -OutFile flixw.java `
  https://github.com/wstein/flixw/releases/download/v0.22.0/flixw.java
java flixw.java install .
...
$ ./flixw validate
ok    ./flixw matches flixw 0.22.0
ok    ./flixw.cmd matches flixw 0.22.0
ok    .flixw/flixw.java  sha256=045464da352d09f9...
...
```console
base=https://github.com/wstein/flixw/releases/download/v0.22.0
curl -fsSLO $base/flixw-0.22.0.tar.gz
curl -fsSL  $base/SHA256SUMS | grep flixw-0.22.0.tar.gz | shasum -a 256 -c -
tar -xzf flixw-0.22.0.tar.gz        # flixw, flixw.cmd, .flixw/flixw.java, .envrc.example
rm flixw-0.22.0.tar.gz
...
On Windows, `Get-FileHash flixw-0.22.0.tar.gz` and `Expand-Archive` are the equivalents.
...
```console
$ ./flixw pin --refresh
flixw: rewrote .flixw/lock.toml in the shape flixw 0.22.0 writes; the pin is unchanged
```
