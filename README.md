...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.33.0/flixw-setup.java
...
```
a4347dbbd2f82e94906adf0667d04941571e727ca6d830ce51b29dfa0459aaac  flixw-setup.java
```
...
```console
echo "a4347dbbd2f82e94906adf0667d04941571e727ca6d830ce51b29dfa0459aaac  flixw-setup.java" \
...
```console
curl -fsSLO https://github.com/wstein/flixw/releases/download/v0.33.0/SHA256SUMS
...
Invoke-WebRequest -OutFile flixw-setup.java `
  https://github.com/wstein/flixw/releases/download/v0.33.0/flixw-setup.java
...
$ ./flixw validate
ok    ./flixw matches flixw 0.33.0
ok    ./flixw.cmd matches flixw 0.33.0
...
```console
base=https://github.com/wstein/flixw/releases/download/v0.33.0
curl -fsSLO $base/flixw-0.33.0.tar.gz
curl -fsSL  $base/SHA256SUMS | grep flixw-0.33.0.tar.gz | sha256sum -c -
tar -xzf flixw-0.33.0.tar.gz        # flixw, flixw.cmd, .flixw/flixw.java
rm flixw-0.33.0.tar.gz
...
On Windows the equivalents are `Get-FileHash flixw-0.33.0.tar.gz` and `Expand-Archive` in
PowerShell, or `certutil -hashfile flixw-0.33.0.tar.gz SHA256` and `tar -xf` in cmd.exe —
...
```console
$ ./flixw pin --refresh
flixw: rewrote .flixw/lock.toml in the shape flixw 0.33.0 writes; the pin is unchanged
```
