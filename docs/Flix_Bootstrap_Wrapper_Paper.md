...
> **Superseded by implementation results.** This paper is kept as historical evidence of
...
```console
java .flix-wrapper/flix.java test
```

Git Bash offers `./flix test` on Windows. The small `flix.cmd` shim works from both Command Prompt and PowerShell; arguments transformed by `cmd.exe` remain documented deviations. `java .flix-wrapper/flix.java test` is the universal Windows fallback and does not depend on PowerShell execution policy.
...
`.flix-wrapper/` is committed wrapper metadata, not a cache or machine-local directory. Compiler JARs and compiled stage-0 classes remain in the user cache. Dedicated root pollution is therefore limited to `flix` and `flix.cmd`; `.gitattributes` is shared repository infrastructure.
...
`flix`, `flix.cmd`, and `.flix-wrapper/flix.java` are byte-identical across repositories for a given wrapper release. `wstein/flixw` can therefore publish one hash per file and version. Project-specific URL and digest data live only in `.flix-wrapper/lock.toml`; updating a compiler pin changes data, not executable code.

Published wrapper hashes apply to canonical release bytes. `.gitattributes` establishes `/flix text eol=lf`, `/.flix-wrapper/flix.java text eol=lf`, `/.flix-wrapper/lock.toml text eol=lf`, and `/flix.cmd text eol=crlf`. Validation distinguishes Git blob identity from checked-out canonical bytes. The installer owns a marked block, replaces it idempotently, preserves unrelated rules, and fails if a later rule overrides these paths. It also sets and verifies the POSIX executable bit and documents archive-download recovery.
...
5. Otherwise, invoke the compiler, so that filenames and future verbs keep working.
   (The original rationale — that Flix owns unknown-command diagnostics — did not survive
   measurement: `flix doctro` answers `Unrecognized file extension: 'doctro'.` on stdout.
   The routing is still correct; the resulting message is not good.)
...
First contact uses a downloaded, release-verified `flix.java` directly; its `install` mode creates `.flix-wrapper/`, writes the invariant files and compiler-only lock, and leaves only `flix` and `flix.cmd` as dedicated root entry files. Thereafter one entry point is used:
...
**P2 — Invariant executable artifacts.** Wrapper implementation files vary only by wrapper release; project data lives in `.flix-wrapper/lock.toml`.
...
1. Specify canonical bytes and hashes for `flix`, `flix.cmd`, and `.flix-wrapper/flix.java`.
2. Specify `.flix-wrapper/lock.toml` and its consistency rules.
...
2. Test `flix.cmd` from Command Prompt and PowerShell plus direct `java .flix-wrapper/flix.java` on Windows.
...
java0 := FLIX_JAVA_HOME/bin/java
      ...
      or java_on_path()
      or fail(FLIXW003)
exec(java0, wrapper_root / ".flix-wrapper/flix.java", original_arguments)
...
lock := parse_generated_lock(root / ".flix-wrapper/lock.toml")
