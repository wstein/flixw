...
# Stock Flix answers `check --help` with the top-level screen, so flixw renders the
# source-backed focused view. The implementation rationale is deliberately not user-facing.
t 0 "help flix renders focused compiler help without a banner" sh -c '
  out=$(./flixw help flix check) &&
  printf "%s" "$out" | grep -Fq "Usage: ./flixw check" &&
  ! printf "%s" "$out" | grep -Fq "publishes only a top-level"'
...
g 0 'not __fish_seen_subcommand_from init check' \
...
# One completion, generated from one picocli CommandSpec. bash, zsh and fish come from
# picocli's own AutoComplete; pwsh walks the same model. The point of the shared model is
# that four shells cannot describe four command sets, so every shell names the same verb.
...
g 0 '# Generated by picocli version' "fish comes from picocli's native generator" ./flixw completion fish
g 0 "complete -c 'flixw' -e" "native fish completion is safe to source repeatedly" ./flixw completion fish
g 0 'not __fish_seen_subcommand_from' "native fish scopes root options before subcommands" ./flixw completion fish
...
t 0  "completion needs no project at all"                       sh -c '
  ...
  cd "$d" && java "$2/src/stage0/flixw.java" completion fish | grep -q "# Generated by picocli version"' sh "$work" "$root"
